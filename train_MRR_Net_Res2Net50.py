...
if __name__ == '__main__':
    nets_name = 'MRR_Net_Res2Net50'
    cfg = Config(datapath='data/yourTrainingDataset',
                 savepath='pths/', mode='train',
                 batch=32, lr=0.05, momen=0.9, decay=5e-4, epoch=50, lr_decay_gamma=0.1)
    data = Data(cfg)
    ...
    for epoch in range(cfg.epoch):
        ...
        if epoch > cfg.epoch * 4 / 5:
            torch.save(net.state_dict(),save_pth_dir+ 'MRR_Net_Res2Net50_' + str(epoch + 1) + '.pth')